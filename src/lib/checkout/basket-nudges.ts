...
import type { DiscountType, Paise } from "../money";
...
export interface OfferNudge {
  ...
  /** Eligible products not yet in the basket, to suggest. */
 ...
 * bigger discount breaks ties. Bundles already applied are skipped.
 */
export function nextOfferNudge(
  ...
  for (const rule of rules) {
    ...
    const suggest = rule.eligibleProductIds.filter((id) => !inBasket.has(id));
    ...
    const candidate: OfferNudge = {
      ...
      suggestProductIds: suggest.slice(0, 4),
    };
    if (
      !best ||
      candidate.missing < best.missing ||
      (candidate.missing === best.missing && rule.discountValue > best.discountValue)
    ) {
      best = candidate;
    }
  }
